...
if (!function_exists('roles_check_permission_by_user_id')) {
    function roles_check_permission_by_user_id($sdkConfig, $module, $action, $user_id)
    {
        $roles = new \nguyenanhung\Platforms\WebBuilderSDK\RolesAndPermission\Components\Roles($sdkConfig['OPTIONS']);
...
if (!function_exists('roles_check_permission_by_user_group')) {
    function roles_check_permission_by_user_group($sdkConfig, $module, $action, $user_group)
    {
        $roles = new \nguyenanhung\Platforms\WebBuilderSDK\RolesAndPermission\Components\Roles($sdkConfig['OPTIONS']);
...
if (!function_exists('roles_check_permission')) {
    function roles_check_permission($sdkConfig, $module, $action, $user_id, $user_group)
    {
